    ...
    async def synthesize_response(
        ...
        try:
            ...
            result = await self.provider.process_text_messages(
                messages=messages,
                max_tokens=800,  # Longer response for synthesis
...
Key Findings: {result.key_findings if result.key_findings else 'No specific findings listed'}
    ...
    def _create_fallback_response(
        ...
        for i, result in enumerate(task_results, 1):
            ...
            if result.key_findings:
                response_parts.append(f"Key findings: {result.key_findings}")
    ...
    async def synthesize_single_result(
        ...
        try:
            ...
            if task_result.key_findings:
                response += f"\n\nKey findings: {task_result.key_findings}"
